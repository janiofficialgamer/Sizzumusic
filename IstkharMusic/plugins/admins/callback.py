...
async def unban_assistant(_, callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer(
            "» ᴍʏ ᴀssɪsᴛᴀɴᴛ ᴜɴʙᴀɴɴᴇᴅ sᴜᴄᴄᴇssꜰᴜʟʟʏ 🥳\n\n➻ ɴᴏᴡ ʏᴏᴜ ᴄᴀɴ ᴘʟᴀʏ sᴏɴɢs 🔉\n\n» ᴛʜᴀɴᴋ ʏᴏᴜ ғᴏʀ ᴜɴʙᴀɴ 💝",
            show_alert=True
        )
    except Exception as e:
        await callback.answer(
            "» ꜰᴀɪʟᴇᴅ ᴛᴏ ᴜɴʙᴀɴ ᴍʏ ᴀssɪsᴛᴀɴᴛ ʙᴇᴄᴀᴜsᴇ ɪ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ʙᴀɴ ᴘᴏᴡᴇʀ\n\n➻ ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴍᴇ ʙᴀɴ ᴘᴏᴡᴇʀ sᴏ ᴛʜᴀᴛ ɪ ᴄᴀɴ ᴜɴʙᴀɴ ᴍʏ ᴀssɪsᴛᴀɴᴛ ɪᴅ",
            show_alert=True
        )
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Istu.pause_stream(chat_id)
        ...
        await CallbackQuery.answer()
        await Istu.stop_stream(chat_id)
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Istu.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Istu.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await Istu.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
